...
Step 1: 仕様と実装の直接突合（verifier）
Step 2: ファイル構成の突合
Step 3: 処理フローの検証（analyzer）
Step 4: 受入条件の検証
Step 5: result.md 生成 + 検証レポート + 次のアクション
```
...
Task(subagent_type: verifier):
  ...
  実装ファイル: {対象ファイル一覧}
  注意: 仕様書は機能仕様書。機能・振る舞いの観点で検証してください。
  {debug コンテキストがある場合}
...
plan.md にデータフロー図（シーケンス図・フロー図）がある場合、analyzer で実装の呼び出しチェーンをトレースし、仕様のフローと突合する。

データフロー図がない場合はスキップ。

```
Task(subagent_type: analyzer):
  プロンプト: 「以下の処理フローが実装で正しく実現されているか検証してください。
  仕様のフロー: {plan.md のデータフロー図セクション}
  実装ファイル: {対象ファイル一覧}
  検証方法:
  1. フローの起点から終点まで、実装の呼び出しチェーンをコードを追ってトレースする
  2. 仕様の各ステップが実装のどの箇所に対応するかをマッピングする
  3. 仕様にあるが実装にないステップ、実装にあるが仕様にないステップを報告する
  報告形式:
  - フローごとに: 仕様ステップ → 実装箇所（ファイル:行番号）の対応表
  - 不一致があれば: どのステップで乖離しているか」
```
