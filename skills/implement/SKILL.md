...
入力: `docs/{feature-name}/plan.md`（設計仕様。読み取り専用）
状態管理: `docs/{feature-name}/progress.md`（実行状態の単一ソース）
出力: 実装コード + PR
...
```
Read docs/{feature-name}/progress.md
```
...
### 0-c. モード判定（progress.md の mode フィールド）
...
1. Task(git-analyzer) でブランチの実態を調査:
   ...
   progress.md: docs/{feature-name}/progress.md
...
**ブランチ名の規則**:
- 形式: `feature/{english-name}-pr{N}`
- 例: `docs/リマインダー/` の PR2 → `feature/reminder-pr2`
