...
4. **既存コードへの影響**（analyzer の「既存コード影響分析」にリスク「高」の項目がある場合） — リスク「高」の影響箇所を列挙し、既存の振る舞いが変わる可能性をユーザーに提示する。リスク「高」がなければ省略
5. **既存環境との不整合**（入力ファイルがある場合のみ） — ソースドキュメントの記載と analyzer が検出した既存環境に矛盾がある場合、事実を提示してユーザーに判断を仰ぐ。矛盾がなければ省略
