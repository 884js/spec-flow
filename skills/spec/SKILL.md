...
2. **関連プラン検出（新規）**: 各プランの frontmatter（title, feature-name）と変更対象ファイル・対象セクションを抽出し、以下の基準で関連候補を検出する:
   ...
   関連候補が1件以上ある場合、AskUserQuestion（multiSelect）で提示する:
   ```
   以下のプランが関連している可能性があります。リンクするプランを選択してください:
   - {feature-name}: {title}（理由: {検出理由}）
   - {feature-name}: {title}（理由: {検出理由}）
   ```
   選択されたプランは `relatedPlans` として保持し、Step 4 で writer に渡す。
   関連候補が0件の場合はスキップする。
